...
const data = (await (await fetch("./unicode.txt")).text()).split("\n").map((x) => x.split(";").slice(0, 2));
// From this list of blocks we should make
/*
[
{ blockname: "", width: 0, height: 0, characters: [
"x:10  y:20  char:  @, | name: FullName"
] }
]
*/
const per = IMAGE_PROPERTIES.characters_per_row;
let BLOCKS_DATA = [];
